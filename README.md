...
7. **Sends Slack alerts** only when something actually needs CM attention, with category, urgency, sentiment, confidence score, AI summary, reason, and a clickable Discord message link
...
28. **Blitz of Battle branded colors**: every Discord embed and Slack message (alerts, reports, trend alerts, member-exit log) uses a consistent orange/light-orange/dark-orange palette instead of generic blue/red, with urgency mapped to shade (critical = dark orange, high = orange, medium/low = light orange)
29. **Exit-feedback reply tracking**: if someone who left the server replies to their exit-feedback DM, the bot recognizes it (via a lightweight per-user record made when the DM was sent) and forwards their exact reply to the configured `DISCORD_DM_USER_IDS` as a new embed -- so it's never just sitting unseen in the bot's own DM inbox. DMs from anyone else (who never received an exit message) are correctly ignored, not treated as feedback
30. **Automatic Notion sync**: set `NOTION_API_KEY` and every new tracked issue creates a Community Management DB entry (matching the manual CM workflow's exact fields: Type, Sentiment, Priority, Status, Platform, Source URL back to the Discord message, Impact Level scaled to reporter count), plus a linked Bugs DB entry for bug reports or a Features DB entry for suggestions. When the same issue gets more reports later, the bot patches the existing Notion page instead of creating a duplicate -- and if urgency crosses into high/critical, it automatically flips Status to "Escalated" and Action Taken to "Escalated to Development" for dev-facing categories, which is the "update it for the dev team" behavior. Off entirely if `NOTION_API_KEY` is unset
31. **`/community peak [window]`**: an activity heatmap for finding the best time to run an event. Tracks every message in watched channels (including casual chat, not just classified feedback -- since raw volume and who's around is the actual signal needed here), converted to a configurable display timezone (`TIMEZONE_OFFSET_HOURS`, default +2 for Cairo). Supports four windows: `24h`, `7d`, `30d` (real dated results, e.g. "Tue Aug 18, 8PM-9PM") or the default `all-time` (recurring weekly pattern, e.g. "Saturday 8PM-9PM"). Ranks the top 5 busiest slots by message volume and unique active participants, recommends the single best window, and adds an AI-written take on the pattern
32. **Response-time tracking**: detects live staff replies by Discord role, in two ways -- if you use Discord's native reply feature to quote the specific message, that exact item is marked answered precisely; otherwise it falls back to "a staff member said something in this channel" and marks every pending item there. Records the real elapsed time either way. `/community pulse` shows a rolling 7-day average response time (Discord-only, ephemeral reply -- never posted to Slack). A background check every 15 minutes finds items pending longer than `STALE_REPLY_HOURS` (default 24) and sends a one-time ⏰ stale alert to your configured Discord DM only, by design -- this is a personal accountability signal, not something broadcast to the shared Slack channel
33. **`/community topic <search>`**: everything mentioning a specific hero, feature, or keyword across a lookback window (default 30 days) -- total mentions, unique players, sentiment split, any tracked issue cluster, and representative quotes with links, plus an AI take. Built for balance/feature discussions where you want "show me everything about X" rather than a time-window digest
34. **`/community todo`**: a personal action queue combining still-unanswered "needs reply" items (sorted by urgency, not just age) with issues flagged for follow-up via `/community assign` -- one place to check instead of splitting attention between Slack and Discord
35. **Moderation alerts for toxicity**: fires independently of the normal urgency/needs-reply gating, since toxic content often doesn't naturally qualify for a regular alert. DM-only (not Slack), and it actually **pings you** (a real @mention in the message, not just text inside an embed -- Discord only notifies from plain content mentions). Deduplicated per channel over the aggregation window, so a whole heated back-and-forth triggers one ping, not one per message
...
| `/community peak [window]` | Best time to run an event -- 24h/7d/30d/all-time |
| `/community topic search:<term> [days]` | Everything mentioning a specific hero/feature/keyword |
| `/community todo` | Your personal action queue: needs-reply items + flagged follow-ups |
