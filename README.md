...
30. **Automatic Notion sync**: set `NOTION_API_KEY` and every new tracked issue creates a Community Management DB entry (matching the manual CM workflow's exact fields: Type, Sentiment, Priority, Status, Platform, Source URL back to the Discord message, Initial Response pre-filled with the AI's suggested reply, Impact Level scaled to reporter count), plus a linked Bugs DB entry for bug reports or a Features DB entry for suggestions. When the same issue gets more reports later, the bot patches the existing Notion page instead of creating a duplicate -- and if urgency crosses into high/critical, it automatically flips Status to "Escalated" and Action Taken to "Escalated to Development" for dev-facing categories, which is the "update it for the dev team" behavior. Off entirely if `NOTION_API_KEY` is unset
