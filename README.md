...
31. **`/community peak`**: an activity heatmap for finding the best time to run an event. Tracks every message in watched channels (including casual chat, not just classified feedback -- since raw volume and who's around is the actual signal needed here) bucketed by day-of-week and hour, converted to a configurable display timezone (`TIMEZONE_OFFSET_HOURS`, default +2 for Cairo). Ranks the top 5 busiest slots by message volume and unique active participants, recommends the single best window, and adds an AI-written take on the pattern
...
| `/community peak` | Best time to run an event, based on peak activity/engagement |
